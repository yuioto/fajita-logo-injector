/* 
 * OP6 Logo Injector aka Logo Injector aka OP3Inject
 *
 * Copyright (C) 2018 Joseph Andrew Fornecker
...
int32_t DecodeLogoBin(FILE *originalLogoBin, IMAGEHEAD *imageHeaders){
	...
	for ( i = 0 ; i < numberOfOffsets ; i++ ){
		...
		fprintf(stdout,
            ", Header=%s, Width=%ld, Height=%ld, Data Length=%ld, Special=%ld, Name=%s, Metadata=%s, ",
            imageHeaders[i].header,
            (long int)imageHeaders[i].width,
            (long int)imageHeaders[i].height,
            (long int)imageHeaders[i].lengthOfData,
            (long int)imageHeaders[i].special,
            imageHeaders[i].name,
            imageHeaders[i].metaData);
    if (convertToPNG){
      ...
      sprintf(name, "%d.png", i);
			lodepng_encode24_file((const char*)name,
                            Decode(originalLogoBin,
                                   (uint32_t)start,
                                   (uint32_t)imageHeaders[i].lengthOfData,
                                   (uint32_t)imageBytes, image),
                            (unsigned)imageHeaders[i].width,
                            (unsigned)imageHeaders[i].height);
			free(image);
		}
	}
	return 0;
...
void Usage(){
	...
	fprintf(stdout, "Usage: OP6Logo -i \"input file\" [-l] | [-L] | [-d [-s]] | [-j \"image to be replaced\" [-b] | [-z] | [-s]]\n\n");
	...
	fprintf(stdout, "\t-z This flag forces the use of OP6 headers, it is detected automatically though.\n");
	...
	fprintf(stdout, "\t   OP6Logo -i \"your_logo.bin\" -j FHD \n\n");
...
int32_t main(int32_t argc, char** argv){
	...
	if (IsItTheOnePlus5(originalLogoBin) == MAXOFFSETS){
		
		fprintf(stdout, "\nThis is a OnePlus 5t image.\n\n");
		z = 1;
	}
	...
	if (z){
		...
		if(inject){
			...
			if (GetFileSize(newModified) > 0x1000000){
				...
				fprintf(stdout, "THE SPLASH PARTITION ON THE ONEPLUS IS NOT BIG \n");
      ...
      char fname[100];
            ...
            sprintf(fname, "%d.png", i);
						if (InjectNewStyle(originalLogoBin, imageHeaders , numberOfOffsets, fname, i, modifiedLogoBin, ihMainOffsets) == 0){
							fprintf(stderr, "Error: Injecting %s\n", fname);
							fclose(originalLogoBin);
							fclose(modifiedLogoBin);
							return 0;
